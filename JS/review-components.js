...
buildVerdict()+
buildCommunity();article.insertAdjacentHTML('beforeend',bottomHTML);const communitySection=article.querySelector('.rt-community');if(communitySection)wireCommunity(communitySection);const footerHTML=buildPrevNext()+buildRelatedGrid();const mount=document.getElementById('rt-related-mount');if(mount){mount.outerHTML=footerHTML;}else{article.insertAdjacentHTML('afterend',footerHTML);}
const scoreFills=article.querySelectorAll('.rt-score-bar-fill');if('IntersectionObserver'in window){const io=new IntersectionObserver(function(entries){entries.forEach(function(entry){if(entry.isIntersecting){entry.target.classList.add('is-visible');if(entry.target.classList.contains('rt-score-panel')){scoreFills.forEach(function(f){f.style.width=f.dataset.target+'%';});}
io.unobserve(entry.target);}});},{threshold:0.15});document.querySelectorAll('.reveal').forEach(function(el){io.observe(el);});}else{document.querySelectorAll('.reveal').forEach(function(el){el.classList.add('is-visible');});scoreFills.forEach(function(f){f.style.width=f.dataset.target+'%';});}})();